...
/*!*****************************************************************************
\brief
	This file contains the Controller class for gamepad controller movements.
    (NOT SUPPORTED IN OUR VR APP)
*******************************************************************************/
...
export enum Type {
  Left,
  Right, // only for triggers and stick event
  Up,
  Down, // only for button events
}
/**
    ...
    gamepadManager.onGamepadConnectedObservable.add((gamepad, state) => {
      ...
      if (gamepad instanceof Xbox360Pad) {
        ...
        gamepad.onleftstickchanged((values) => {
          stickText.text =
            "x:" + values.x.toFixed(3) + " y:" + values.y.toFixed(3);
